...
```bash
kbb --backend sci tools/verify.cljk
```
...
```bash
kbb --backend sci tools/verify.cljk --preflight
```
...
kbb --backend sci tools/verify.cljk > /tmp/out.log; echo "exit=$?"   # → exit=1
...
kbb --backend sci tools/verify.cljk > /tmp/out.log; echo "exit=$?"   # → exit=2
...
kbb --backend sci tools/verify.cljk --preflight > /tmp/out.log 2>&1
...
`kbb --backend sci tools/verify.cljk | grep FINDING; echo $?` は `grep` の値を読む。
